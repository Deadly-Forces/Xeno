...
export default function AnalyticsPage(): JSX.Element {
  ...
  const decision = useQuery<Decisioning>({
    queryKey: ["decisioning"],
    queryFn: async () => {
      const response = await fetch("/api/decisioning");
      if (!response.ok) throw new Error("Unable to load decision model");
      return response.json() as Promise<Decisioning>;
    },
  });
