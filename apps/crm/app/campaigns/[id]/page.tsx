...
export default function CampaignDetailPage(): JSX.Element {
  ...
  const { data: session } = useQuery<{ user?: { role?: string } }>({ queryKey: ["session"], queryFn: async () => (await fetch("/api/auth/session")).json() as Promise<{ user?: { role?: string } }> });
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex gap-2"><button className="btn" disabled={control.isPending || (data.campaign.guardrailPaused && session?.user?.role !== "ADMIN")} onClick={() => control.mutate(data.campaign.guardrailPaused ? "resume" : "pause")}>{data.campaign.guardrailPaused ? <Play size={16} /> : <Pause size={16} />}{data.campaign.guardrailPaused ? session?.user?.role === "ADMIN" ? "Approve resume" : "Admin approval required" : "Pause"}</button><a className="btn" href={`/api/campaigns/${params.id}/export`} download><Download size={16} /> Export CSV</a></div>
      </header>
