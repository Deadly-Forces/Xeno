...
export async function GET(_: Request, context: { params: { id: string } }): Promise<Response> {
  ...
  const segment = await db.segment.findFirst({ where: { id: context.params.id, organizationId: actor.organizationId }, select: { filterRules: true, customerCount: true } });
  ...
  const [countResult, customers] = await Promise.all([
    countPromise.then((count) => ({ count, cached: false })).catch(async () => ({ count: Number(await redis.get(`segment:${context.params.id}:count`) ?? segment.customerCount), cached: true })),
  ...
  if (!countResult.cached) {
    await Promise.all([
      redis.set(`segment:${context.params.id}:count`, String(countResult.count), "EX", 300),
      db.segment.update({ where: { id: context.params.id }, data: { customerCount: countResult.count }, select: { id: true } })
    ]);
  }
