...
function RuleGroupEditor({ group, onChange, depth = 0 }: { group: SegmentGroup; onChange: (group: SegmentGroup) => void; depth?: number }): JSX.Element {
  ...
  return <div className={depth ? "rounded-md border border-line bg-[#f8faf9] p-3" : "space-y-3"}>
    <div className="mb-3 flex flex-wrap items-center gap-2"><span className="text-sm text-[#65736e]">Match</span><select aria-label="Rule group operator" className="input w-24" value={group.operator} onChange={(event) => onChange({ ...group, operator: event.target.value as "AND" | "OR" })}><option>AND</option><option>OR</option></select><span className="text-sm text-[#65736e]">of these rules</span></div>
    <div className="space-y-2">{group.rules.map((rule, index) => <div key={index} draggable onDragStart={() => setDragged(index)} onDragOver={(event) => event.preventDefault()} onDrop={() => move(index)} className="flex min-w-0 items-start gap-2">
      ...
      <div className="min-w-0 flex-1">{isGroup(rule) ? <RuleGroupEditor group={rule} depth={depth + 1} onChange={(nested) => replace(index, nested)} /> : <div className="grid grid-cols-[1.2fr_1fr_1.3fr] gap-2"><select aria-label={`Rule ${index + 1} field`} className="input" value={rule.field} onChange={(event) => replace(index, { ...rule, field: event.target.value as SegmentCondition["field"] })}>{fields.map((field) => <option key={field}>{field}</option>)}</select><select aria-label={`Rule ${index + 1} operator`} className="input" value={rule.operator} onChange={(event) => replace(index, { ...rule, operator: event.target.value as SegmentCondition["operator"] })}>{operators.map((operator) => <option key={operator}>{operator}</option>)}</select><input aria-label={`Rule ${index + 1} value`} className="input" value={Array.isArray(rule.value) ? rule.value.join(",") : rule.value} onChange={(event) => replace(index, { ...rule, value: parseValue(rule, event.target.value) })} /></div>}</div>
