...
function ToolResultCard({ part }: { part: ToolPart }): JSX.Element {
  ...
  if (toolName === "recommend_campaign") {
...
export function AIChatSidebar({ onDraft, onRecommendation, onSegmentCreated }: { onDraft?: (draft: { template: string; channel: string }) => void; onRecommendation?: (channel: string) => void; onSegmentCreated?: () => void } = {}): JSX.Element {
  const { messages, sendMessage, status, error } = useChat({ transport: new DefaultChatTransport({ api: "/api/ai/chat" }) });
  ...
  useEffect(() => {
    for (const message of messages) for (const part of message.parts) {
      ...
      if (toolPart.state !== "output-available" || handled.current.has(toolPart.toolCallId) || !isToolResult(toolPart.output)) continue;
      handled.current.add(toolPart.toolCallId);
      const toolName = getToolPartName(toolPart);
      const result = toolPart.output as ToolResult;
      if (toolName === "draft_message") onDraft?.({ template: String(result.template ?? ""), channel: String(result.channel ?? "EMAIL") });
      if (toolName === "recommend_campaign") onRecommendation?.(String(result.channel ?? "EMAIL"));
      if (toolName === "create_segment") onSegmentCreated?.();
    }
  ...
  return <aside className="flex h-[calc(100vh-10rem)] min-h-[560px] min-w-0 flex-col overflow-hidden border-l border-line bg-white">
    ...
    <div className="min-w-0 flex-1 space-y-4 overflow-y-auto overflow-x-hidden p-4">
      ...
      {messages.map((message) => <div key={message.id} className={`min-w-0 max-w-[92%] overflow-hidden rounded-md p-3 text-sm ${message.role === "user" ? "ml-auto bg-[#183b33] text-white" : "border border-line bg-white"}`}>
        {message.parts.filter((p): p is { type: "text"; text: string } => p.type === "text").map((p, i) => p.text && <p key={i} className="whitespace-pre-wrap break-words leading-6">{message.role === "assistant" ? cleanAssistantText(p.text) : p.text}</p>)}
        {message.parts.filter(isToolPart).map((p) => <ToolResultCard key={(p as ToolPart).toolCallId} part={p as ToolPart} />)}
      </div>)}
